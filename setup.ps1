...
<#
.SYNOPSIS
    Setup script for @ms-cloud-experts/mcp-business-central - MCP server for Business Central
.DESCRIPTION
    Checks prerequisites, runs Azure login, and writes the Claude Desktop config
    to launch the MCP server via npx. No git clone, no npm install, no build.
#>
...
Write-Host "  |   MS Cloud Experts edition                       |" -ForegroundColor Cyan
Write-Host "  +==================================================+" -ForegroundColor Cyan
Write-Host ""
...
# Build BC_URL_SERVER
$baseUrl = "https://api.businesscentral.dynamics.com/v2.0/$bcTenantId/$bcEnvironment/api"
if ($bcCustomPub) {
    $bcUrlServer = "$baseUrl/$bcCustomPub/$bcCustomGroup/$bcCustomVer"
} else {
    $bcUrlServer = "$baseUrl/v2.0"
}

Write-Ok "BC_URL_SERVER: $bcUrlServer"
Write-Ok "BC_COMPANY:    $bcCompany"
...
Write-Host "    3. First launch will take a few seconds to download the package" -ForegroundColor Gray
Write-Host "    4. Look for the hammer icon with the BC tools" -ForegroundColor Gray
...
Write-Host "  To update, re-run this script or let npx pull the latest version." -ForegroundColor Yellow
Write-Host ""
